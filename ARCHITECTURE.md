...
| `src/lib/terminal-theme.ts` | Pure design-tokens → xterm theme mapping, every color converted to parse-safe hex (`xtermSafeColor`: modern `hsl()` → `#rrggbb[aa]` — xterm's own parser silently drops any non-opaque color outside `#hex`/legacy `rgb()`, which lost the alpha selection token). Keeps the terminal surface truthful to PADE's scheme — for the user's eyes. An already-running agent does **not** follow it on Windows: Claude's `auto` theme is `live override ?? $COLORFGBG ?? dark`, the override is set only by an OSC 11 answer, and that query never reaches the terminal here, so the spawn env decides the scheme for the session's whole life (`theming.rs`). `Terminal` still relays the DECSET 2031 `?997` report — the documented channel, and harmless — but measurement says it does not move a live Claude | `Terminal`, `terminal-color-scheme` |
...
| `theming.rs` | Forces each installed agent's own startup syntax theme to ADE's resolved scheme because Windows ConPTY consumes the agent's initial OSC 11 query. Every mechanism is registry data: per-scheme env, per-scheme args, a PADE-owned TUI config, and an optional native project seed (`ProjectThemeSeed { relative_path, contents }`) applied by one generic create-only `ensure_project_theme` path. Claude selects adaptive `theme:auto`, receives `COLORFGBG`, and seeds a missing `.claude/settings.local.json` — and because that env pair is read once, at spawn, the scheme the frontend hands `pty_spawn` must already be the real one: `prefs.svelte` seeds it from the native `window_theme` before mount rather than from WebView2's media query; Codex uses `-c tui.theme=…`; OpenCode uses `OPENCODE_TUI_CONFIG` to select a per-scheme PADE theme (`pade-light` / `pade-dark`, plain-string so its ConPTY-answered probe can't spoil the render); Cursor/aider use env. Copilot follows the OS itself, Grok inherits the terminal palette, and Antigravity has no verified safe override, so PADE does not guess settings. **No agent has a working live channel on Windows** — Claude's `?997` report is only a prompt to re-probe OSC 11, and that query never reaches the terminal (measured; see `docs/terminal-rendering.md` rejected item 5) — so a scheme flip is followed the only way it can be: App's `restartSpawnThemedAgents` respawns the **idle** sessions of all three spawn-themed agents (Claude, Codex, opencode), each resuming its own conversation. Claude was excluded from that until its `--session-id` respawn race was re-measured against 2.1.220 (start → kill → respawn on the same id at 0/250/1500ms: all survive) and found gone. A session that is *busy* at the moment of the flip is skipped and keeps its spawn scheme until it next restarts |
