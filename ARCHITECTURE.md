...
| `theming.rs` | Forces each installed agent's own startup syntax theme to ADE's resolved scheme because Windows ConPTY consumes the agent's initial OSC 11 query. The registry declares the spawn-time mechanism: per-scheme env applied by `pty.rs` (Claude's and opencode's `COLORFGBG` — opencode's own `system` theme adapts to it, aider's `AIDER_LIGHT/DARK_MODE`, cursor-agent's `TERM_THEME`) or per-scheme launch args (Codex's `-c tui.theme=catppuccin-latte|mocha`, overriding its config only for that launch). For Claude's `auto` theme, `Terminal` additionally sends its DECSET 2031 `?997` report through the live PTY on every appearance change, so it redraws with its context intact; other agents retain their spawn syntax choice while xterm updates immediately |
