...
| `src/lib/AppMenu.svelte` | Top-bar project switcher: an Open-windows list (focus any window; cycle with Ctrl+Shift+Alt+[ / ]), a filter (Ctrl P — a capture-phase shortcut so the terminal can't swallow it), pinned/recent rows with truncation-`title` tooltips on clipped paths and a per-row kebab (pin/unpin, remove-from-list, delete-directory) and drag-reorderable pins, then open-a-project / new-window actions. The kebab's **delete-directory confirms in place**: it owns a `nested` `ConfirmDialog` that sits inside this popover, so the switcher stays visible (dimmed) behind the prompt and the row collapses out (`motion.collapseRow`) when the parent's `ondelete` resolves — the shell only performs the removal | `bridge`, `drag-reorder`, `ConfirmDialog`, `motion`, `truncation-tooltip` |
