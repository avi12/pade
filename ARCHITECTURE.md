...
| `theming.rs` | Forces each installed agent's own startup syntax theme to ADE's resolved scheme, because ConPTY consumes the agent's OSC 11 background query and xterm's `?997` color-scheme report. The registry declares the spawn-time mechanism: per-scheme env applied by `pty.rs` (Claude's `COLORFGBG`, aider's `AIDER_LIGHT/DARK_MODE`, cursor-agent's `TERM_THEME`) or per-scheme launch args (Codex's `-c tui.theme=catppuccin-latte|mocha`, overriding its config only for that launch). A running agent retains that choice until it naturally exits, while `Terminal` applies the app palette immediately in place — appearance changes never terminate an agent or discard its context |
