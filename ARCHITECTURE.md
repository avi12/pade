...
| `src/lib/SessionTabs.svelte` | Session tab strip: pill/dot/"+N" tiers, off-layout measurement, add-agent menu; each full tab's agent glyph is tinted by its context-window fill (the `--context-*` gauge — colour only, no number) | `tab-fit`, `stores/sessions`, `stores/context`, `context-level`, `agent-icon` |
