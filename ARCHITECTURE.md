...
| `src/lib/task-detect.ts` | Pure `isTaskInvocation` — conservative proof that a known task was launched: a shell-prompt command or an agent `PowerShell(command)`/`Bash(command)` tool call, after stripping ANSI (`ansi`); summaries merely mentioning a command never set it running | `stores/taskRuns` |
...
| `src/lib/stores/taskRuns.svelte.ts` | Reflects a known task an agent runs (conservatively spotted in its PTY output via `task-detect`) or a live runner dock launches as "running" in the Tasks panel; agent status clears when its session goes idle/exits, while runner status follows the process lifecycle |
