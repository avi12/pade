...
| `src/lib/stores/context.svelte.ts` | Per-session context-window fill from two signals: the agent's own parsed readout, else a rough PTY-byte estimate. `contextPercentage` (parsed-or-estimate) tints the soft tab gauge; `measuredContextPercentage` (parsed only) is what auto-handoff / usage-resume / API-error retry gate on — the byte estimate over-counts a fullscreen agent's repaints, so it must never end a session. A tokens counter alone is not enough: it needs the window as its denominator, and agents no longer announce one in their banner, so `Terminal` re-asks `model_window` (via `contextWindowKnown`) at every idle until the answer exists — the model is read from the agent's own session log, which carries none until its first turn has been written |
...
| `model_window.rs` | A session's context-window size when its startup `(N context)` banner isn't in the replay (a long re-attached session, where the frontend can't parse the window and the gauge/early-handoff stall). `agent_context_window` reads the MODEL off disk — Claude records `"model"` per-conversation in `~/.claude/projects/*/<conversationId>.jsonl` — and sizes it from the live **models.dev** catalog (`api.json`, public, flattened to model→largest `limit.context`, fetched once per run behind a 6h TTL). Nothing about model→window is hardcoded. Only Claude is resolved on purpose: Codex's only on-screen token counts are tool output, so a window would re-arm the false handoff `context.svelte.ts` guards against. `Terminal` calls it on attach → `seedContextWindow` (never overriding a live banner reading) |
