...
| `src/lib/TerminalFind.svelte` | Find-in-terminal for one pane: the Ctrl+F bar that floats over the output (absolutely positioned, so the measured viewport — and with it the PTY's grid — never changes when it opens). Owns its open state (the pane calls `reveal()`), the search term as a validated field (`TerminalFindTerm`), and xterm's search addon, imported on first open so a session that never searches never loads it. Enter / Shift+Enter step, Escape closes and hands the keyboard back; highlight colours come from `terminal-theme` and are re-applied when the palette flips. Searches xterm's ACTIVE buffer — a shell's scrollback, or just the visible frame while a fullscreen agent holds the alternate screen | `terminal-find`, `terminal-theme`, `validate`, `Icon`, `Terminal` |
