...
| `theming.rs` | Forces each installed agent's own startup syntax theme to ADE's resolved scheme because Windows ConPTY consumes the agent's initial OSC 11 query. The registry declares the spawn-time mechanism: per-scheme env applied by `pty.rs` (Claude's `COLORFGBG`, aider's `AIDER_LIGHT/DARK_MODE`, cursor-agent's `TERM_THEME`), per-scheme launch args (Codex's `-c tui.theme=catppuccin-latte|mocha`, overriding its config only for that launch), or a **spawn-time TUI-config file** for a CLI with neither knob (opencode, whose light/dark detection rides ConPTY-poisoned OSC 10/11 replies): on a light scheme `theming.rs` materializes a PADE-owned tui config (merged over the user's own tui.json via `OPENCODE_TUI_CONFIG`) plus the additive, mode-independent `pade-light` theme it selects, and `pty.rs` sets the env pair; dark needs no override. For Claude's `auto` theme, `Terminal` additionally sends its DECSET 2031 `?997` report through the live PTY on every appearance change, so it redraws with its context intact; other agents retain their spawn syntax choice while xterm updates immediately |
