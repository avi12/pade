...
| `src/panels/ChangeFeed.svelte` | Live file-change feed, grouped by project (`change-groups`) under role-badged headers that carry the workspace's git branch as a subtitle (`vcs.branchOf`, refreshed on mount / project switch) with a per-project chip filter, a change-kind filter, and per-card file-type chips (`file-type`); expanding a card fetches the git-free session-baseline preview (`feed.diff`) and renders it through the shared `unifiedDiff` → `parseDiff` → `DiffView` path. The toolbar's **Sync all** button fast-forwards the workspace from `origin` (`vcs.pull`) with an in-flight spinner and a status toast |
