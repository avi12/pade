...
| `theming.rs` | Forces each installed agent's own startup syntax theme to ADE's resolved scheme because Windows ConPTY consumes the agent's initial OSC 11 query. Every mechanism is registry data: per-scheme env, per-scheme args, a PADE-owned TUI config, and an optional native project seed (`ProjectThemeSeed { relative_path, contents }`) applied by one generic create-only `ensure_project_theme` path. Claude selects adaptive `theme:auto`, receives `COLORFGBG`, and seeds a missing `.claude/settings.local.json`; Codex uses `-c tui.theme=…`; OpenCode uses `OPENCODE_TUI_CONFIG` plus `pade-light`; Cursor/aider use env. Copilot follows the OS itself, Grok inherits the terminal palette, and Antigravity has no verified safe override, so PADE does not guess settings. Claude's live DECSET 2031 `?997` report still redraws a running session in place |
