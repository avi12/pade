...
| `src/lib/session-restart.ts` | Pure logic behind restarting agents in place (new session id, conversation resumed) when a project's MCP configuration changes: `mcpRestartTargets` picks the governed sessions whose working dir IS the changed dir (a worktree keeps its own config) and that carry a resumable `conversationId`; `rekeyLayout` re-keys those sessions across the pane layout (new id → terminal remounts and resumes; `initialPrompt` dropped). `App` owns the reactive side (kill PTYs, toast, assign); these stay pure and unit-tested | `paths`, `types`, `App` |
...
| `config.rs` | Surface (read-only) the config files each agent CLI uses — the one registry of "which file, which agent, which kind". Reads resolve from the calling window's registered project, never process-global cwd. `mcp_configs()` exposes the MCP-kind entries (`.mcp.json` → Claude) so `watcher.rs`/`mcp.rs` watch the right file without a second list |
