...
| `src/lib/stores/handoff.svelte.ts` | Document handoff orchestration: near-limit scan, handoff-doc wait, successor launch, consumed-doc deletion (via the narrow `handoff_doc_delete` command), a `force()` entry for usage recovery, and an MCP configuration path that pins the successor to the same governed agent so its fresh process loads the changed config before continuing. Every path holds while `stores/agentActivity` says the agent still has work running — the scan skips such a session (with a status note) and re-runs when the report clears, and a started handoff waits both before typing its request and before ending the session — so no handoff severs a dynamic workflow or background agent that a quiet, `ready`-looking session is still waiting on. Ends the predecessor through the shell's `endSession` rather than `pty.kill` — the shell respawns a replacement for an *unannounced* last-session exit, so an unflagged handoff kill raced its own exit event into a bare extra tab that also stole the active pane from the successor being seeded behind it |
