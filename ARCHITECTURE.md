...
| `ide.rs` | Editor detection + user-added editors, per-kind suggestion rules, open-at-line; `ProjectKind` is the single typed source-language registry (markers, extensions, Linguist language names, UI label via `ide_kinds`) and each registered editor declares its language coverage separately. Kind detection probes markers in the root and one level down; a bounded byte census profiles tracked source (Git root paths, binary/generated content excluded) while Git resolves the project’s `.gitattributes` `linguist-*` exclusions and language overrides. Suggestions rank every installed editor by byte-weighted coverage, then marker coverage and capability breadth — no dominant-language cutoff or framework-name rule. A configured per-language rule leads only when its editor covers the complete observed language profile: WebStorm can lead an all-web monorepo but not a web/Rust hybrid. `family()` also flags console editors that run in a terminal tab |
