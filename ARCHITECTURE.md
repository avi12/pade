...
| `src/lib/repository-links.ts` | Opens a project's backend-normalized remote in the system browser and surfaces missing remotes or opener failures through the shared toast. A central provider registry maps branch URLs for GitHub, GitLab, Bitbucket, Codeberg/Gitea, and Azure DevOps; unknown hosts safely open the repository root. Its `openRepositoryOnModifiedClick` attachment preserves the project menu's plain click while Ctrl/Cmd-click opens the repository | `App`, `AppMenu`, `bridge`, `stores/toast` |
