...
| `ide.rs` | Editor detection + user-added editors, per-kind suggestion rules, open-at-line; `ProjectKind` is the single typed project/source-family registry (markers, extensions, Linguist language names, UI label via `ide_kinds`) and each registered editor declares its language coverage separately. A Tauri configuration is a first-class composite project marker, ordered before its web/Rust components so a WebStorm web rule never applies to the desktop app. Kind detection probes markers in the root and one level down; a bounded byte census profiles tracked source (Git root paths, binary/generated content excluded) while Git resolves the project’s `.gitattributes` `linguist-*` exclusions and language overrides. Suggestions rank every installed editor by byte-weighted coverage, then marker coverage and capability breadth — no dominant-language cutoff; a specialist leads a one-language project, while a general editor wins whenever it is the only complete fit. `family()` also flags console editors that run in a terminal tab |
