...
| `src/panels/ConfigPanel.svelte` | Read-only view of the active agent's config files plus app performance preferences; software rendering persists through the shared prefs store and offers an explicit restart because WebView2's `--disable-gpu` switch is launch-only |
