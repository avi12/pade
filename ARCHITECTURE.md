...
| `src/lib/repository-links.ts` | Opens a project's backend-normalized remote in the system browser and surfaces missing remotes or opener failures through the shared toast. Its `openRepositoryOnModifiedClick` attachment lets the project-menu trigger preserve its normal popover click while Ctrl/Cmd-click opens the repository root | `AppMenu`, `bridge`, `stores/toast` |
