...
| `terminal_schemes.rs` | The Windows Terminal colour schemes PADE can paint its terminal with (`terminal_schemes`): the ones Windows Terminal ships — vendored as an asset, since its `defaults.json` lives inside an app package that may not be installed or readable — merged with the ones the user wrote into their own `settings.json` (Store, Preview, and unpackaged builds), the user's winning by name exactly as Windows Terminal resolves them. Read through `util::read_jsonc`, because that file is one a human edits and the vendor's own template ships with comments. On the wire the palette is an ANSI-indexed array, not Windows Terminal's twenty named fields: index *is* the SGR colour, which is the order `ANSI_COLOR_TOKENS` maps onto the design tokens. One half-typed scheme costs only itself |
