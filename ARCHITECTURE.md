...
PADE wraps an AI coding-agent CLI (Claude Code, Codex, opencode, …) running **unmodified in
...
The webview is treated as untrusted input, not an authorization boundary. A
restrictive CSP blocks remote code/resources; native commands re-check paths,
object ids, URLs, sizes, and ownership. PTYs, task runners, config reads, and
watchers are bound to the calling window so one project window cannot inspect or
control another.

WebView2 browser arguments are resolved before Tauri creates any window. On
Windows, `lib.rs` reads the persisted passthrough `softwareRender` preference
(with `PADE_SOFTWARE_RENDER=1` as an override) and adds `--disable-gpu`; because
that switch is launch-only, Config persists the choice and invokes `os::restart`
through the bridge only when the user explicitly asks to restart.
...
- **Runs, not lines.** Only the columns that actually read right to left are covered
  ...
  into one. A run reaches as far as the bidi algorithm itself would — through a number
...
owns this file" is on disk: a **manifest** makes a folder a package, a **`.git`**
makes it a checkout. `members.rs` finds both in one census walk and reports them
as a single list, each member flagged `repository` when it has git of its own.

The two are confirmed differently, and deliberately so. A package must be
*declared*: the root's workspace-defining files contribute the include/exclude
patterns, and a manifest-holding directory is a member IFF a pattern claims it.
A checkout declares itself: nobody lists nested repos anywhere, so the marker is
the whole test — which is why a checkout the workspace's own `exclude` array
rejects is still a member (Cargo's exclude speaks about a build, not about what
is in the folder).

That one list drives everything downstream. `change-groups.ts` assigns each
changed file to its deepest enclosing member by whole-segment longest-prefix —
`apps/web` never captures `apps/web-admin` — with the repo root as the
always-present fallback bucket. `workspace-members.ts` resolves the member a
group belongs to and the **enclosing checkout** whose branch it should show, so a
monorepo package reports the repo that contains it and a nested checkout reports
itself. `stores/workspaceMembers` holds the list, those branches, and which
member the workspace is pointed at — the top bar's switcher, the branch pill and
the working directory of a new agent or the editor. A workspace with no confirmed
members keeps the old `apps/`·`packages/`·`services/` folder-name convention and
no switcher at all, so single-repo behavior is unchanged.
...
flowchart LR
  ...
  Walk["one census walk<br/>manifest? · own .git?<br/>(node_modules · target · dot-directories pruned)"] --> Confirm{"a package a pattern claims,<br/>or a checkout of its own?"}
  Globs --> Confirm
  Confirm -->|yes| Members["members_list → stores/workspaceMembers"]
  Members --> Assign["change-groups.ts<br/>changed file → deepest member<br/>(whole-segment longest-prefix)"]
  Members --> Switcher["MemberSwitcher · BranchSwitcher pill<br/>new agent · worktree · editor cwd"]
  Assign --> Feed["ChangeFeed groups<br/>(branch of the enclosing checkout)"]
```
...
| `src/theme.css` | M3 Expressive tokens, the Cyberpunk palette's two faces and the skin that reinterprets the app in them, global keyframes, base document styles, and the cross-cutting shells many components share by class name — the anchored `.popover-menu` with the `.menu-separator` / `.menu-divider` a menu puts between its rows, `[popover]`, the `.ck` checkbox, the `[data-tooltip]` bubble | everything |
| `src/App.svelte` | App-shell orchestrator: phase routing (loading → picker / onboarding / ready), spawned-window boot, reload re-attach (a boot first re-adopts the backend's still-live sessions via `session-restore`, before any stale `w=` query routing — and the shell keeps that `w=` query rewritten to the project actually on screen, so even a snapshot-less reload lands right), session list + split panes, launch flows, auto-close-on-exit (respawn the same agent when the last session self-exits, e.g. Ctrl-C; a never-named temp workspace instead returns to the picker and is deleted), in-window project switch, leave-to-picker and an intercepted window close (deliberate leaves — one at a time — gracefully kill every session of the project being left — idle-first via `whenSessionIdle` — so no agent keeps running, or cwd-locking, a workspace the window has moved on from), side-panel host; adopts `settings://changed` (`workspace.onChanged`) so settings another window moved — a workspace it deleted, a pin it toggled — land here too instead of this window drifting until its own next mutation; wires the extracted concerns below | `SessionTabs`, panels, `auto-name`, `session-restore`, `stores/handoff`, `workspace-relocate`, `send-shortcut`, `tab-shortcuts`, `stores/toast` |
| `src/lib/SessionTabs.svelte` | Session tab strip: pill/dot/"+N" tiers, off-layout measurement, add-agent menu; each full tab's agent glyph is tinted by its context-window fill (the `--context-*` gauge — colour only, no number) and flashes red (steady red under reduced motion) while its agent waits on a multiple-choice answer, until that tab is looked at or answered. Pointer, close-button, and app-level Ctrl+W closes all route through `closeSession`, so the same pill-collapse outro runs before its marker is retired by `outroend` | `tab-fit`, `stores/sessions`, `stores/context`, `stores/sessionAttention`, `context-level`, `agent-icon`, `App` |
| `src/lib/AppMenu.svelte` | Top-bar project switcher: an Open-windows list (focus any window; cycle with Ctrl+Alt+[ / ]), a filter (Ctrl P — a capture-phase shortcut so the terminal can't swallow it), pinned/recent rows with truncation tooltips on clipped paths (via the shared top-level overflow bubble) and a per-row kebab (pin/unpin, relabel-in-PADE for a temp workspace, remove-from-list, delete-directory) and drag-reorderable pins, then open-a-project / new-window actions. **Up/Down walk the project items** in visual order — window rows, the filter field, then pinned/recent rows (`arrowFocus` over the `[data-arrow-stop]` set, so every row stays in the natural Tab order alongside its kebab); an open kebab menu takes the arrows for itself (`rovingMenu`). A drag grip's tooltip comes from the shared top-layer bubble too — its row is named for the reorder morph, and that name would trap a `[data-tooltip]` pseudo in the row's paint order. Open windows and branch chips are fetched on each open; each `ProjectKindIcon` independently joins the shared lazy batch only when its row intersects the viewport, so Git branch work never gates language icons and offscreen projects cost no marker probe. **Every list change animates**: each row carries a stable `view-transition-name` and the mutation runs inside a view transition scoped to the rows container (`switchList.startViewTransition` — never `document`, so the live-repainting terminal is never snapshotted/ghosted), so a pinned row glides Recent→Pinned and a removed/deleted row morphs out (reduced motion runs the update directly). The kebab's **delete-directory confirms in place**: it owns a `nested` `ConfirmDialog` that sits inside this popover, so the switcher stays visible (dimmed) behind the prompt; the shell only performs the removal. **Relabel** (a temp row's kebab, or the Save card's Relabel button for the current temp) opens the shared `RelabelDialog` nested the same way | `bridge`, `ProjectKindIcon`, `drag-reorder`, `roving-menu`, `ConfirmDialog`, `RelabelDialog`, `truncation-tooltip` |
| `src/lib/RelabelDialog.svelte` | Relabel a temp workspace inside PADE — the name the top bar, switcher, picker and Discord presence show (all through `paths.displayName` over `settings.labels`) — without renaming its folder, which a live agent holds as its cwd. Validates through `validate.WorkspaceLabel` (any single line ≤ 60 chars, non-Latin script and path characters welcome) and saves with `workspace_set_label` as `LabelSource.manual`, which always wins over auto-naming. One prompt for both entry points: `AppMenu` opens it `nested`, the picker's `WorkspaceDialogs` as a modal | `bridge.workspace`, `ConfirmDialog`, `validate` |
...
| `src/lib/MemberSwitcher.svelte` | The top bar's "which part of this workspace?" control, shown only when the workspace has more than one member. Lists every member with its branch when it is a checkout; picking one is what the branch pill reports and where a new agent tab, a worktree launch and the editor open. Reads the shared store, so it can never disagree with the Change Feed's grouping | `stores/workspaceMembers`, `workspace-members`, `Icon` |
| `src/lib/BranchSwitcher.svelte` | The top bar's branch pill **and the one control that moves HEAD**: it names the branch the pointed-at member is on and drops a menu of every branch the repo can switch to — the local ones (checked), then the ones only a remote has yet, which `vcs.switchBranch` reaches by letting git create the local tracking branch. Git's own refusal (uncommitted work in the way, a branch already held by another worktree) is what the toast says, because that sentence names the obstacle; the shell re-reads the branch state after a switch, and `git://state` keeps the pill honest when HEAD moves outside the app. Ctrl-click still opens the branch on its remote — the menu's last two rows spell that out, and the Git panel | `bridge.vcs`, `repository-links` (via `App`), `roving-menu`, `stores/toast`, `Icon` |
| `src/lib/TerminalFind.svelte` | Find-in-terminal for one pane: the Ctrl+F bar that floats over the output (absolutely positioned, so the measured viewport — and with it the PTY's grid — never changes when it opens). Owns its open state (the pane calls `reveal()`), the search term as a validated field (`TerminalFindTerm`), the three match rules (case / whole word / regex, per pane, flipped by click or Alt+C/W/R), and xterm's search addon, imported on first open so a session that never searches never loads it. A rule change re-runs the search through `clearDecorations()` first — the addon stores the options it was handed and then compares the next call's options against that same object, so a changed rule alone never invalidates its highlighted set. Enter / Shift+Enter step, Escape closes and hands the keyboard back; highlight colours come from `terminal-theme` and are re-applied when the palette flips. Searches xterm's ACTIVE buffer — a shell's scrollback, or just the visible frame while a fullscreen agent holds the alternate screen | `terminal-find`, `terminal-theme`, `validate`, `Icon`, `Terminal` |
...
| `src/lib/validate.ts` | User-input schemas (trust boundary) + `parseInput` / `nameError`; owns the clone-URL shape knowledge — `CloneUrl` (https / ssh / scp-like), `GitUsername`, `GitSecret`, `isSshCloneUrl`, `repositoryFolderName` | form components |
...
| `src/lib/task-detect.ts` | Pure `isTaskInvocation` — conservative proof that a known task was launched: a shell-prompt command or an agent `PowerShell(command)`/`Bash(command)` tool call, after stripping ANSI (`ansi`). The command is matched as a separator-bounded token run *inside* the invocation, so it fires the way an agent actually runs a task — `Bash(cd app && pnpm dev 2>&1)` — yet never mistakes `pnpm dev` for `pnpm dev:prod`; summaries merely mentioning a command never set it running | `stores/taskRuns` |
...
| `src/lib/auto-name.ts` | Temp-workspace auto-naming: distinct-file counting, once-per-workspace naming call, saved as `LabelSource.auto` — so the backend never lets it replace a label already there (the user's own relabel included) | `bridge.feed/workspace`, `paths` |
| `src/lib/workspace-relocate.ts` | Move/rename/delete/**save-migrate** with cwd-lock handling: kill locking sessions → backend op → resume remapped (delete has nothing to resume and drops the project). `saveMigrate` is the temp→real-project save: same lock release, then `workspace_save_migrate` copies the files, the agents resume on the new dir, and the emptied temp is deleted (best-effort); it hands the detected install command back to `App` to run in a runner pane | `bridge`, `stores/sessions`, `stores/context` |
...
| `src/lib/change-groups.ts` | Pure grouping of Change Feed events into project buckets, summing each group's line deltas. Ground truth first: with manifest-confirmed workspace members (`bridge.members` ← `members.rs`) a change buckets under its **deepest enclosing member** (whole-segment longest-prefix, so `apps/web` never captures `apps/web-admin`; outside every member → the repo). Only a workspace with no confirmed members falls back to the folder-name convention — a change under an `apps/`·`packages/`·`services/` container groups by its member **folder** (an `@scope/name` kept whole) — a loose file sitting in the container itself (`app/WindowChrome.cs`) is the repo's, not its own one-file project — else the repo itself. Each group also carries its **cards**: a run of consecutive changes to one file collapses into one entry (`repeats`, with the run's summed deltas), so an agent saving a file four times reads as one card saying `×4` instead of four identical ones — only a RUN folds; a file changed again later is its own card | `ChangeFeed`, `bridge.members` |
...
| `src/lib/feed-status.ts` | Pure `WatchStatus` → the sentence the Change Feed's empty panel shows: nothing has changed yet (naming the watched root and how long it has been armed, plus how many changes the project's ignore rules hid), the watch is armed on a *different* project, or there is no watch at all — the last two flagged `stalled` so the panel offers to arm it. An empty list is otherwise ambiguous, and the panel used to word all three the same | `ChangeFeed` |
| `src/lib/format.ts` | Locale-aware number formatting, plus `formatAge` — the one home for the relative ages the UI prints (a feed card's "3m ago", the empty panel's "watching for 12m") | UI counts/stats |
| `src/lib/usage-groups.ts` | Pure per-agent usage model: running sessions → deduped, worst-first `AgentGroup`s (per-agent limits when its account resolves, else "unknown"), the severity/spotlight/legend view-model (incl. the formatted credits balance on the extra-usage row), and the agent→icon map | `UsageMeter` |
...
| `src/lib/roving-menu.ts` | The vertical sibling of `roving-tabs`: Up/Down (wrapping) and Home/End move focus between a menu's items — never activating; the click/Enter does that. One keydown wiring behind two attachments — `rovingMenu` (items are the menu's `<button>`s, which carry a roving tabindex so one Tab leaves, and focus lands on the first item when the popover opens) and `arrowFocus(selector)` (items keep their natural tabindex, for a menu whose rows sit among other controls). Both stand down while a nested popover is open — that menu owns the keys — and leave Home/End to a focused text field's caret, so a filter input can be one of the stops. `nextIndex` is the pure, unit-tested index math | `SessionTabs` add-session menu, `AppMenu` |
...
| `src/lib/workspace-members.ts` | Pure reading of a workspace member: where it lives on disk (the backend reports root-relative paths), what to call it (manifest name → own folder → the workspace's folder), which member a Change Feed group belongs to, and the **enclosing checkout** whose branch that group should show — a monorepo package reports the repo that contains it, a nested checkout reports itself | `stores/workspaceMembers`, `MemberSwitcher`, `ChangeFeed`, `paths` |
| `src/lib/terminal-find.ts` | Pure find-bar vocabulary: the Ctrl+F chord matcher (plain Ctrl only, so an agent keeps Ctrl+Shift+F and Ctrl+Alt+F), the search direction enum, the match-rule set as one table (option key — the addon's own — plus glyph, label and Alt chord, so a button, its tooltip and the key that flips it cannot describe three different things), whether a term compiles as a pattern, and the result readout ("1 of 12", "No matches", or the total alone once the addon stops tracking which match is active) | `TerminalFind`, `tab-shortcuts` (chord shape) |
...
| `src/lib/theme-mode.ts` | Pure resolution of the theme the user picked into the two things the document root wears: the light/dark `scheme` (`data-theme`) and the token set over it (`data-palette`). "Light"/"Dark" name a scheme, "Auto" takes the OS's, and "Cyberpunk" names a palette and takes its scheme from the OS too — which is why the picker's four options are not four schemes | `prefs.svelte`, `ConfigPanel` |
| `src/lib/terminal-palette.ts` | Pure mapping of a chosen Windows Terminal scheme onto the `--terminal-*` design tokens (plus the terminal surface and caret), and `schemeOfBackground` — light or dark as the scheme's OWN background answers it, which is what the agent is then told. Installing a scheme writes tokens rather than talking to xterm, so every ANSI surface (the terminal, the runner dock's SGR parser) stays on one palette; `TERMINAL_PALETTE_PROPERTIES` is the same list the uninstall clears | `prefs.svelte`, `stores/terminalSchemes` |
| `src/lib/terminal-theme.ts` | Pure design-tokens → xterm mapping — the terminal's theme and the find bar's match/active-match highlights (`--terminal-find-*`, kept translucent because the addon paints the active one OVER the glyphs), plus the one DOM read both resolve their tokens through (`rootTokenReader`); every color converted to parse-safe hex (`xtermSafeColor`: modern `hsl()` → `#rrggbb[aa]` — xterm's own parser silently drops any non-opaque color outside `#hex`/legacy `rgb()`, which lost the alpha selection token). Keeps the terminal surface truthful to PADE's scheme — for the user's eyes. A running agent never follows this palette through the protocol on Windows: the OSC 11 answer that would carry it never crosses ConPTY in either direction, so `Terminal`'s DECSET 2031 `?997` relay stays (documented, harmless) but moves nothing. What moves a live Claude is its watched theme definition, rewritten by `theming.rs` | `Terminal`, `terminal-color-scheme` |
...
| `src/lib/settings-state.ts` | Pure replacement and monotonic-version primitives behind settings adoption; replacement removes optional keys absent from a fresh backend value and versions reject superseded responses | `settings.svelte`, tests |
...
| `src/lib/truncation-tooltip.ts` | Two Svelte attachments over one shared bubble: `tooltip(text)` always surfaces its text on hover/focus, and `truncationTooltip` surfaces it only while an element's text is actually clipped — measured with the DOM's own `scrollWidth > clientWidth` on `pointerenter` (settled layout, exact). Instead of a local `title` or `::after`, it tags the trigger with a unique `anchor-name` and reports its text to the shared overflow-tooltip state on hover/focus, so the one top-level bubble positions itself back onto the trigger. Use it wherever a `[data-tooltip]` pseudo would be clipped or occluded — inside a scroller, a `content-visibility` subtree, or an element carrying a `view-transition-name` | `overflow-tooltip`, `AppMenu`, `ChangeFeed`, picker `RecentSection` |
| `src/lib/overflow-tooltip.svelte.ts` | The single active-truncation-tooltip target (`{ anchorName, text }`) plus show/clear helpers; a stale trigger only clears the bubble if it still owns it | `truncation-tooltip`, `OverflowTooltipLayer` |
| `src/lib/OverflowTooltipLayer.svelte` | Renders that one bubble as a `popover="manual"` element (the TOP LAYER, above the switcher's own popover — a plain fixed div would paint behind it), anchored back to the active trigger by `anchor-name`. Mounted once at the App root so it escapes any panel/popover clipping or scroll-width inflation. Solves the reason clipped paths used a native `title` | `overflow-tooltip`, `App` |
...
| `src/lib/stores/agentActivity.svelte.ts` | What the agent itself reports about its work, read from its terminal title in the PTY stream (and the history replayed on attach): Claude Code's `◐`/`◑` glyph while a turn, background agent or dynamic workflow runs, `✳` once none does — rewritten on every change even when its unfocused spinner stops ticking. A title split across chunks is carried to the next one. `agentReportsBusy` (reactive) and `whenAgentSettled` (event-driven, settles on the idle glyph or when `App` drops the session) are the gate session-ending flows wait behind; an agent that announces no glyph reports nothing, leaving every gate open |
...
| `src/lib/stores/context.svelte.ts` | Per-session context-window fill from three signals, strongest first: the agent's own session log (`observeSessionLog`, fed by `session_context`), else its parsed on-screen readout, else a rough PTY-byte estimate. `contextPercentage` (any of the three) tints the soft tab gauge; `measuredContextPercentage` (log or parsed, never the estimate) is what auto-handoff / usage-resume / API-error retry gate on — the byte estimate over-counts a fullscreen agent's repaints, so it must never end a session. The log outranks the screen because the screen is mostly silent: Claude Code paints no percentage until the last ~3% of the window, and the screen-scraped tokens counter that fills the gap counts what *streamed*, not what the prompt *occupies* (a 47%-full session read 32%). `Terminal` re-reads the log at every idle — the fill grows all session long, and a fresh session's log carries neither model nor usage until its first turn is written |
| `src/lib/stores/handoff.svelte.ts` | Document handoff orchestration: near-limit scan, handoff-doc wait, successor launch, consumed-doc deletion (via the narrow `handoff_doc_delete` command, and only once the successor's own `stores/agentActivity` report shows its first turn ran and finished — a successor that reports no turn keeps its doc rather than lose it unread), a `force()` entry for usage recovery, and an MCP configuration path that pins the successor to the same governed agent so its fresh process loads the changed config before continuing. Every path holds while `stores/agentActivity` says the agent still has work running — the scan skips such a session (with a status note) and re-runs when the report clears, and a started handoff waits both before typing its request and before ending the session — so no handoff severs a dynamic workflow or background agent that a quiet, `ready`-looking session is still waiting on. The successor stays the same agent while `hasUsageHeadroom` says it can work — a headline quota that is not fully spent, or usage credits (`CreditsState.available`) carrying it past that cap — and crosses over to another installed agent only when it truly cannot. Ends the predecessor through the shell's `endSession` rather than `pty.kill` — the shell respawns a replacement for an *unannounced* last-session exit, so an unflagged handoff kill raced its own exit event into a bare extra tab that also stole the active pane from the successor being seeded behind it |
| `src/lib/stores/mcpReload.svelte.ts` | Bounded PTY-output recovery detector for same-name MCP definition edits: only sessions armed by a config edit are scanned; a reported MCP reload failure escalates through `App` to the safe same-agent document handoff, while a successful reload clears the pending state |
| `src/lib/stores/usageResume.svelte.ts` | Usage-limit auto-resume: sniffs the CLI's usage-limit stop from the PTY stream and the rendered screen, confirms it against the agent's account usage, then waits (never nudges) — re-reading the account each minute until the window resets or usage credits can carry the session — and continues it once: "continue" in place, or `handoff.force()` when the context is nearly full; nothing is typed if the screen already shows a turn running. Owns the usage-limit classifier (`isUsageLimitStop`) and `CONTINUE_PROMPT` that the API-error retry defers to |
| `src/lib/stores/recoveryLoop.svelte.ts` | The bookkeeping the automatic-recovery loops share (usage-limit resume, API-error retry): one pending timer per session, the status note, the scan that starts a freshly flagged session and forgets a closed one, and teardown — each loop supplies its own detection state and what one tick does |
| `src/lib/stores/runners.svelte.ts` | Task-runner rows + backend stream subscription. Also **attached** runners — cards for a task the agent started itself: no captured output (PADE never owned the process), but Stop kills it via `pty.sessionTaskStop`, and a liveness poll (`pty.sessionTaskRunning`) drops the card once the process exits, so a dev server's card outlives the agent's turn |
| `src/lib/stores/taskRuns.svelte.ts` | Reflects a known task an agent runs (conservatively spotted in its PTY output via `task-detect`) or a live runner dock launches as "running" in the Tasks panel; agent status clears when its session goes idle/exits, while runner status follows the process lifecycle. On the same detection it opens an **attached** dock runner (`stores/runners` `attachRunner`) — process-lifetime, not turn-lifetime — so the run gets a stoppable card |
| `src/lib/stores/terminalSchemes.svelte.ts` | The Windows Terminal colour-scheme catalogue (`terminal_schemes`), loaded once per window before the first appearance apply, plus `chosenTerminalScheme` — the one resolution from a persisted NAME to the scheme itself, so the list the picker renders and the palette actually painted can never be two different answers. A name that no longer exists resolves to "follow the app theme" rather than a ghost |
| `src/lib/stores/sidePanel.svelte.ts` | Active side-panel header (count + refresh action) |
| `src/lib/stores/workspaceMembers.svelte.ts` | What the open workspace is made of (SSOT): one `members_list` per project, the HEAD branch of every member that is a checkout (`vcs_branch_of`, refreshed on `git://state`), and **which member the workspace is pointed at** — the directory a new agent tab, a worktree launch and the editor open in. The switcher, the branch pill and the feed's grouping all read this one list, so a workspace can never be two shapes at once |
...
| `src/lib/stores/projectKinds.svelte.ts` | Shared reactive project-kind cache: one `IntersectionObserver` lazily queues visible `ProjectKindIcon` rows into a single `ide_project_kinds` batch, normalized paths deduplicate callers, and immediate evidence probes publish into the same cache used by the switcher and Discord presence |
| `src/lib/stores/new-project-detection.ts` | One-shot coordination for newly created workspaces: listens to the existing `feed://change` stream, debounces created-file bursts, asks the shared project-kind store whether a registry marker now exists, refreshes the editor ranking once, then permanently retires that workspace's detector |
...
| Module | Responsibility |
| --- | --- |
| `src/panels/Terminal.svelte` | xterm.js DOM terminal bound to one PTY session; deliberately avoids xterm's optional WebGL addon so terminals allocate no dedicated GPU contexts or vRAM, and disables cursor blinking while hidden or unfocused without pausing PTY parsing. PTY signals are handled immediately, but bytes are coalesced before xterm writes: one flush per foreground animation frame at the live bottom or every 250ms in the background and while the user reads scrollback; a wheel gesture that scrolls **xterm's own document** defers writes until 120ms of quiet with a one-second ceiling (`wheelScrollsTerminalDocument`) — never when the agent owns the mouse or the tick is forwarded as PageUp/PageDown, because there the wheel is input and the agent's repaint *is* the scroll, so deferring it is the lag. Every byte and ordering survive while live output stops fighting scroll frames. WebView2 native occlusion throttling remains enabled so covered windows suspend visual work. Owns the document-style reflow (grid fit, anchor, settle-debounced `SIGWINCH`) and refuses transient undersized layout measurements so a dock/panel change never resizes Codex to a broken tiny grid. Makes plain-text URLs clickable via `terminal-links` (whole wrapped URL → system browser) and OSC-8 hyperlinks via its own `linkHandler` (web → browser, local `file:` paths → Explorer); themes xterm from the design tokens (`terminal-theme`) on every scheme flip without replacing the live PTY; intercepts Ctrl+F for the pane's find bar (`TerminalFind`) so it never reaches the agent as a raw ^F and, after Claude enables DECSET 2031, relays the dark/light report (which on Windows Claude treats only as a prompt to re-probe OSC 11 — a query that never arrives — so a live session keeps the scheme it was spawned with). **Delivers a new session's first prompt**: rather than firing it on mount (where it would collide with the agent's first-run trust gate), it watches the stream — auto-accepts the trust gate (`initial-prompt`), then once the agent has settled at its REPL pastes the prompt as a bracketed paste and submits it with a separate Enter |
| `src/panels/ChangeFeed.svelte` | Live file-change feed, grouped by project (`change-groups`, fed the manifest-confirmed members from `members.list` — refetched on a project switch, convention fallback on failure) under role-badged headers that carry the workspace's git branch as a subtitle (`vcs.branchOf`, refreshed on mount / project switch and live on `git://state` — a branch switch or `git init` updates it in place) with a per-project chip filter and a **file-type filter** — a pill that drops a native popover of per-extension checkboxes with live counts (tallied once from the feed via `file-type` `fileExtension`, SSOT) plus a Select-all toggle — and per-card file-type chips (`file-type`); expanding a card fetches the git-free session-baseline preview (`feed.diff`) and renders it through the shared `unifiedDiff` → `parseDiff` → `DiffView` path. An **image change** (png/jpg/gif/webp/avif/bmp/ico/svg — `lib/preview` `isImagePath`) instead renders an inline preview: the file's bytes fetched as a base64 data URL (`feed.image` ← `feed_image`) and shown via `<img loading=lazy>` on a theme-aware checkerboard, only on expand — SVG goes through the same `<img>` data URL, never inlined as markup, so it runs no scripts. A **markdown or HTML change** (`lib/preview` `isMarkdownPath`/`isHtmlPath`) gets a **Code | Preview** toggle: Code is the diff (default), Preview renders the file's current text (`feed.text` ← `feed_text`) inside a **sandboxed `<iframe>`** (no `allow-scripts`/`allow-same-origin`, plus a `default-src 'none'` srcdoc CSP) — markdown is turned to HTML by the hand-rolled `lib/markdown` (no parser dependency; the PowerToys approach translated to the web) wrapped in a vendored M3 stylesheet, an HTML file is shown as-is; both inert, so untrusted project content runs no scripts and loads no external refs. With no events the panel states its own condition rather than a generic wait: `feed.status` (← `watch_status`) is read on mount, on a project switch, and again whenever the window is looked at again while the list is still empty, and `lib/feed-status` turns it into the headline + evidence — including a **Watch this project** button when nothing is armed on the open workspace (the backend's refusal, which names the path and the reason, surfaces as a toast). The toolbar's **Sync all** button (shown only for a repo with a remote, re-checked live on `git://state`) fast-forwards the workspace from `origin` (`vcs.pull`) with an in-flight spinner and a status toast |
...
| `RowMenu.svelte` | Shared kebab popover: reveal actions + owned-workspace lifecycle entries (relabel-in-PADE for a temp workspace, rename, move, delete) |
| `lifecycle.svelte.ts` | Owned-workspace rename/move/delete flows + inline-rename form state, shared by Recent and Roots; owns the delete confirmation state (target / in-flight / error) and which temp workspace is being relabelled, both rendered by `WorkspaceDialogs` |
| `WorkspaceDialogs.svelte` | The lifecycle's prompts, one of each for the whole picker: the delete `ConfirmDialog` and the PADE-only `RelabelDialog` (modal) |
...
| `workspace.rs` | Settings, roots, temp workspaces, labels, move/rename/delete. `workspace_set_label` takes a `LabelSource`: an `auto` name is kebab-cased (`naming::sanitize`) and lands only on an unlabelled workspace, a `manual` one is kept as typed (`manual_label`: one line, ≤ 60 chars) and always wins — the check-and-set runs under the settings lock, so an auto-name finishing after the user relabelled can't overwrite them. One mutex-protected settings repository serializes every in-process load-modify-save and writes through a same-directory temporary file; preference writes patch fields (null removes an optional key) instead of replacing all prefs. `launch_directory` is the one home for "which directory this instance launched into" (an explicit `pade <dir>` argument, else the process cwd) — read by both `launch_context` (what the frontend boots into) and `webview_data_dir`, which keys this instance's **WebView2 user-data folder** by that project (a stable digest of the canonical path under `%LOCALAPPDATA%\pade\webview2\`) so two projects open in parallel run in **separate browser + GPU process trees** instead of sharing one; `lib.rs` sets `WEBVIEW2_USER_DATA_FOLDER` to it before any window is built (the shared default lets one instance's GPU load and the ~16 WebGL-context cap compound into the other and trip a DWM reset). Adding a root goes through `workspace_add_root` (existing dir → added; missing → created only when the picker asks; a file → rejected) with `workspace_probe_path` feeding the add-row's live check (is-dir / is-file / parent-exists) and its directory autocomplete (child dirs matching the typed prefix) — existence checks in place of a path regex. Deleting first steps the process out of the folder (opening a project chdirs into it, and Windows won't delete the directory a process stands in), then retries briefly while the OS closes the killed agents' handles; an already-absent folder counts as deleted, so a stale Recent row can always be cleared. **Saving a temp workspace** goes through `workspace_save_migrate`, not a rename: on Windows PADE's own recursive watcher holds the temp directory open, so `rename`/move fails with a sharing violation (os error 32), but reads are unaffected — so it **copies** the workspace into the chosen root (skipping `provision.rs` dependency directories), returns the detected install command, and leaves the temp for the frontend to delete once it has opened the copy and resumed the agent there. An empty workspace (saved before any prompting) is the same path — it copies nothing and yields a valid empty project. Because settings are process-wide but each webview holds its own copy, the repository's `update` reports whether the mutation actually moved anything and `update_settings` — the one choke point every mutation passes — then broadcasts payload-free **`settings://changed`** app-wide, so a project deleted, renamed or pinned in one window stops being listed in the others (every window re-reads through its own `settings_get`, keeping one adoption path). A mutation that changes nothing (the picker's recurring prune) neither writes nor announces, so two windows can never refresh off each other's announcements forever |
| `refs.rs` | After a move: re-point agent memory dirs, IDE recents, symlinks, package-manager installs |
| `provision.rs` | The one authoritative table pairing each ecosystem's re-downloadable **dependency directories** (`node_modules`, `.venv`, `vendor`, …) with the **install command** that restores them (`is_dependency_directory` / `detect_install`, chosen from the project's lockfile→manifest order). `workspace_save_migrate` reads both halves so they can't drift — the copy skips exactly what the reinstall will bring back — covering npm/pnpm/yarn/bun, uv/poetry/pipenv/pip, composer, bundler, go |
| `naming.rs` | Temp-workspace auto-naming (agent CLI → heuristic, shared sanitizer) and live-session naming from the terminal transcript. The transcript is first collapsed to its distinct content lines (`readable_transcript`) so a fullscreen agent's repeated screen repaints — spinner, status bar, key hints — can't drown the actual conversation. Session naming tries `copilot.rs` first, then the session's own agent CLI |
| `agents.rs` | Agent registry + detection, one-shot headless invocations, the env each agent is spawned with, and each interactive session's skip-permission arguments. It is also the single source for theme preparation: a `ThemeConfig` selects per-scheme env, launch args, a PADE-owned TUI config, or — for the one agent with a live channel — a PADE-owned theme *definition* selected per session (`SpawnSelectedLiveTheme`: Claude launches on `--settings '{"theme":"custom:pade"}'`, and rewriting `~/.claude/themes/pade.json` re-themes it where it stands). Future agents declare their mechanism in one registry entry without new theming control flow. `program()` resolves agent executables, and `session_id_flag` declares targeted conversation resume. Fullscreen-capable agents are pinned to the alternate screen where supported |
| `theming.rs` | Forces each installed agent's own UI theme to ADE's resolved scheme, because Windows ConPTY consumes the agent's OSC 11 background query in both directions. Every mechanism is registry data. **Claude is re-themed live**: it launches on a PADE-owned theme definition (`--settings '{"theme":"custom:pade"}'`, never a settings file of the user's) and `publish_live_themes` rewrites `~/.claude/themes/pade.json` on every flip — Claude watches that directory and repaints, so a conversation in flight follows the app's appearance without being restarted (`agent_theme_publish` is the frontend's call; `pty.rs` publishes before each spawn so the first frame is already right). The scheme lives in the file's contents, not the argv, which is precisely why it can reach a running process. Everyone else is spawn-themed and read once at launch: Codex uses `-c tui.theme=…`; OpenCode uses `OPENCODE_TUI_CONFIG` to select a per-scheme PADE theme (`pade-light` / `pade-dark`, plain-string so its ConPTY-answered probe can't spoil the render); Cursor/aider use env. Because those are read at spawn, the scheme the frontend hands `pty_spawn` must already be the real one: `prefs.svelte` seeds it from the native `window_theme` before mount rather than from WebView2's media query. The scheme it hands over is the **terminal's**, not the app chrome's (`appearance.terminalScheme`) — with a Windows Terminal scheme chosen the two genuinely differ (a Solarized Light terminal inside a dark app), and the agent paints into the terminal. Copilot follows the OS itself, Grok inherits the terminal palette, and Antigravity has no verified safe override, so PADE does not guess settings. For the spawn-themed pair a flip is followed the only way it can be: App's `restartSpawnThemedAgents` respawns the **idle** ones, each resuming its own conversation; a session that is *busy* keeps its spawn scheme until it next restarts. Claude must never join that set — its `--session-id` respawn wedged live sessions twice — and no longer needs to |
| `resume.rs` | Resume args for agents whose CLI can reopen a recorded conversation but gives ADE no way to pick the id at spawn: `agent_resume_args` resolves Codex's newest *interactive* rollout (`~/.codex/sessions/**/rollout-*.jsonl`, first-line `session_meta`; oneshot `exec` rollouts skipped) whose recorded `cwd` matches the session's, yielding `["resume", "<uuid>"]`, and opencode's newest recorded session for the directory (queried through `opencode db` — stdin nulled, wait capped, child killed on overrun — so a wedged lookup can never hang a spawn), yielding `["--session", "<ses_…>"]` or no flag when nothing is recorded (a blind `--continue` walks opencode's legacy history and throws its #28486 server error) — how a scheme-flip restart (App's `restartSpawnThemedAgents`) respawns either agent on the new theme without losing its conversation. Claude needs none of this: its conversation is pinned at spawn via `--session-id` |
| `session_context.rs` | How full a session's context window is, read off disk instead of scraped off the terminal — the terminal is not a reliable source, since Claude Code renders no percentage until the last ~3% of the window. `agent_session_context` answers both halves from Claude's own per-conversation log at `~/.claude/projects/*/<conversationId>.jsonl`: USED from the newest turn's `usage` block (input + cache read + cache write + output, i.e. everything the next request re-sends), found by a backwards tail scan that widens 64 KB → 1 MB → 8 MB rather than parsing a growing multi-megabyte transcript; and the WINDOW from the `"model"` the same log records, sized by the live **models.dev** catalog (`api.json`, public, flattened to model→largest `limit.context`, fetched once per run behind a 6h TTL). Nothing about model→window is hardcoded. Only Claude is resolved on purpose: Codex writes no per-turn accounting and its only on-screen counts are tool output, so a window would re-arm the false handoff `context.svelte.ts` guards against. `Terminal` re-reads it at every idle → `observeSessionLog` (the window never overriding a live banner reading) |
| `usage.rs` | Per-agent usage / quota meter: for each running agent, the live rate-limit windows its own vendor exposes, read WITHOUT spending quota. Claude → `api.anthropic.com/api/oauth/usage` (`~/.claude/.credentials.json`), Codex → `chatgpt.com/backend-api/wham/usage` (`~/.codex/auth.json`), Copilot → GitHub `api.github.com/copilot_internal/user` (plan + premium-request quota, from the CLI's OAuth store). Antigravity/Cursor show a local plan label when logged in (their usage endpoints are private/fragile/inaccurate — deliberately not polled); Grok/Aider run on the user's own pay-as-you-go key, so an honest "—" (opencode has no usage source wired, so likewise "—"). Claude's response also yields a `CreditsState` (off / available / unavailable) from its `extra_usage` object, which auto-resume reads to continue a limit-stopped session once credits can carry it; its spend cap is classified as the `credits` window kind, and while credits are on the organization's prepaid balance (`/organizations/{uuid}/prepaid/credits`, uuid from `~/.claude.json`) rides along as `credit_balance`. One `UsageAgent` enum + `from_id`, a per-agent 3-min cache, and `usage_get`/`usage_account_agent` dispatching by agent; the meter may explicitly bypass that cache once at an advertised reset boundary so an expired utilization cannot survive another cache cycle. The token always rides a curl `--config` file on stdin (out of argv) and is charset-validated. Never fabricates — a missing token or a non-usage/error body yields `None` (an honest "—"), with a local tier label as the offline fallback |
...
| `tasks.rs` | Discover runnable tasks from project manifests. One registry (`MANIFESTS`) is the sole authority: each entry says how its file is recognised — an exact name (`package.json`, `Cargo.toml`, `Makefile`, `pyproject.toml`, `CMakeLists.txt`, `CMakePresets.json`, `meson.build`, `build.ninja`, `MODULE.bazel`/`WORKSPACE`, `xmake.lua`, `premake5.lua`, `SConstruct`, `go.mod`, `build.gradle[.kts]`, `pom.xml`) or an extension, since C++ and C# name the manifest after the project (`.sln`/`.slnx`/`.csproj`/`.fsproj`/`.vbproj` → `dotnet`, `.vcxproj` → `msbuild`, `.pro` → `qmake`) — plus its family and how to read its tasks. A solution is claimed by whichever family its projects belong to (it is read for a `.vcxproj` reference); a `CMakeLists.txt` or `meson.build` only counts where it declares its own `project()` (an `add_subdirectory`/`subdir()` child builds with its parent) and a `.pro` only where it assigns a qmake variable, since the extension is common outside Qt; `CMakePresets.json` contributes the project's own named configurations (hidden presets are inherited building blocks, not choices); Gradle and Maven run the repo's committed wrapper when there is one; and where a toolchain has two configurations worth a button, both are offered — CMake gets Release twice because a single-config generator takes the build type at configure time and a multi-config one at build time. Commands are written to run unattended: `xmake -y`, `dotnet watch --non-interactive`, `msbuild -restore`, and `ctest -C Debug` on Windows, where a Visual Studio tree finds no tests without a named configuration. A .NET project's **own entry-point targets** are listed after the standard verbs (`dotnet build <project> -t:<name>`), scanned tag-wise out of the project file only — never out of an `<Import>`, since the registry decides what the frontend watches and an imported `.targets` file is not watched, so its targets would never refresh; internal (`_…`), hooked (`BeforeTargets`/`AfterTargets`) and standard-verb names are skipped. `tasks_descriptors` derives the frontend's watched tokens and empty-state list from that same registry, tagged `name` or `extension` so the catalog matches a changed path the way the backend matches a directory entry |
| `terminal_schemes.rs` | The Windows Terminal colour schemes PADE can paint its terminal with (`terminal_schemes`): the ones Windows Terminal ships — vendored as an asset, since its `defaults.json` lives inside an app package that may not be installed or readable — merged with a short curated asset (the light schemes that vendored file lacks: Selenized Light, Catppuccin Latte), then with the ones the user wrote into their own `settings.json` (Store, Preview, and unpackaged builds). Later sources win by name, so the user's beat curated and curated beat shipped, exactly as Windows Terminal resolves a user override. The curated list is a separate asset so regenerating the vendored one from an installed Windows Terminal can never drop it. Read through `util::read_jsonc`, because that file is one a human edits and the vendor's own template ships with comments. On the wire the palette is an ANSI-indexed array, not Windows Terminal's twenty named fields: index *is* the SGR colour, which is the order `ANSI_COLOR_TOKENS` maps onto the design tokens. One half-typed scheme costs only itself |
| `mcp.rs` | Parses a registered agent project config into an existence/content/server-name snapshot: JSON object maps (`mcpServers` or OpenCode's `mcp`) and Codex TOML `mcp_servers` tables share one typed seam. A half-written file reads as `None`, so the watcher skips transient saves rather than treating every server as removed |
...
| `members.rs` | Workspace member discovery (`members_list`) — what the workspace is made of, and the ground truth behind the top bar's member switcher, the branch pill and the Change Feed's grouping. A member is either a **package** (IFF the folder holds its own manifest, never because of its name) or a **repository** (IFF it holds its own `.git` — a nested checkout, submodule or worktree, which no workspace file declares anywhere, so the marker alone confirms it and no include pattern has to claim it; each member reports `repository` so the frontend shows git chrome only where there is git). One census walk records every directory holding a real manifest (`package.json`/`Cargo.toml`/`go.mod`/`pyproject.toml`; `node_modules`/`target`/`dist`/`build`/`vendor`/dot-dirs pruned); the root's workspace-defining files contribute include/exclude member patterns through one parser-per-file seam (`pnpm-workspace.yaml` `packages:` list, `package.json` `"workspaces"` array/object, `Cargo.toml` `[workspace]` `members`/`exclude`, `go.work` `use` directives, `pyproject.toml` `[tool.uv.workspace]`) — all hand-parsed (line scans + a quote-aware mini-TOML reader; serde_json for JSON), no yaml/toml dependency; a census directory becomes a member only when a segment-aware glob includes it (`*`/`?` within one segment, `**` recursive, excludes subtract **after** all includes) and the root is always a member, each carrying its manifest's declared name and ecosystem |
| `runner.rs` | Window-owned task-runner execution with owner-targeted streamed output and per-window process/command caps |
| `clipboard.rs` | Clipboard **image** paste for terminal sessions: `clipboard_image_save` reads the OS clipboard bitmap (the clipboard-manager plugin's Rust API — a TUI in a PTY can't reach the OS image clipboard itself), encodes it to PNG (`png` crate — already in the dependency tree via the plugin's arboard) under `%TEMP%\pade-paste\`, prunes stale saves, and returns the path — which `Terminal`'s Ctrl+V pastes into the agent's composer so it attaches the file; no image on the clipboard falls back to the text paste |
| `config.rs` | Surface (read-only) the config files each agent CLI uses — the one registry of "which file, which agent, which kind/parser". Reads resolve from the calling window's registered project, never process-global cwd. `mcp_configs()` exposes Claude/Copilot JSON, OpenCode JSON, Codex TOML, and Cursor JSON entries so `watcher.rs`/`mcp.rs` watch and parse the right file without parallel lists |
| `design.rs` | Quick-launch AI design/UI tools by registry id; the backend resolves the HTTPS URL so renderer input cannot navigate the companion webview to another origin |
...
| `os.rs` | Reveal in file manager / terminal, open URLs, and request a normal app restart so launch-only preferences take effect through the regular PTY teardown path |
| `window.rs` | Spawn additional app windows (painted with the themed M3 surface so they open in-theme, no white flash); track each window's project and focus/list/**reorder**/cycle between them (`window_focus_project`, `window_list`, `window_focus_label`, `window_focus_relative`, `window_reorder`) — powering the switcher's Open-windows section and the Ctrl+Alt+[ / ] cycle. A user-defined explicit label order (drag-grip reorder in the switcher, session-scoped in `WindowProjects`) is the one source both `window_list` and `window_focus_relative` sort by, so the drag order drives the list and the cycle alike; a window absent from it appends in creation order. Also answers **`window_theme`** — the window's OS theme, which the frontend seeds its scheme from at startup: WebView2's `prefers-color-scheme` can report light in an already-dark window (it resolves after the first script runs) and only a genuine flip would ever correct it, so a session spawned in that gap themed its agent for the wrong scheme permanently. `scheme_for` is the one theme→`"light"`/`"dark"` mapping, shared by this command and the `theme://changed` flip event |
...
| `process-priority/` | Tiny Windows FFI workspace crate behind a safe `lower_current_process` API. `lib.rs` calls it before WebView2 or PTY creation, so PADE, agents, and agent-launched tools inherit below-normal scheduling and yield CPU to other applications |
| `copilot.rs` | Anonymous Copilot (`copilot.microsoft.com`) as the preferred session-name source: a guest chat over a TLS WebSocket authorized by the cookie `POST /c/api/start` mints — no Microsoft account, no bundled SDK, no browser. Solves the `hashcash` proof-of-work, sends the naming prompt, reassembles the `appendText` deltas. Best-effort and cross-platform: any failure (offline, region/consent gate, or the Cloudflare Turnstile bot-check on the first message) returns `None`, so `naming.rs` falls back to the agent CLI. Runs on the naming worker thread, so its blocking HTTP/WS never touches the UI thread |
| `util.rs` | Cross-cutting helpers: executable resolution (`search_dirs`, `find_in`, `resolve`, `is_on_path`), `command` (windowless child processes), `spawn_detached` (a fully detached GUI-app launch — Windows job/console breakaway with a no-breakaway fallback — so a spawned editor outlives PADE; used by `ide.rs`), `home_dir`, `encode_project`, `percent_encode`, and `SKIPPED_DIRECTORIES` / `is_noise_directory` — the one list of build-output and dependency directories both project walks prune (`naming.rs`'s file census, `tasks.rs`'s manifest scan) |
...
- The context-percent regexes are couplings to the **agent CLI's** observable
  output (documented at their definitions) — a deliberate, accepted Hyrum
  dependency, not a stable PADE contract.
