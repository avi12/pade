...
| `tasks.rs` | Discover runnable tasks from project manifests. One registry (`MANIFESTS`) is the sole authority: each entry says how its file is recognised — an exact name (`package.json`, `Cargo.toml`, `Makefile`, `pyproject.toml`, `CMakeLists.txt`, `go.mod`, `build.gradle[.kts]`, `pom.xml`) or an extension, since .NET and MSVC name the manifest after the project (`.sln`/`.csproj`/`.fsproj` → `dotnet`, `.vcxproj` → `msbuild`) — plus its family and how to read its tasks. A `.sln` is claimed by whichever family its projects belong to (it is read for a `.vcxproj` reference), a `CMakeLists.txt` only counts where it declares its own `project()`, and Gradle/Maven run the repo's committed wrapper when there is one. `tasks_descriptors` derives the frontend's watched tokens and empty-state list from that same registry, tagged `name` or `extension` so the catalog matches a changed path the way the backend matches a directory entry |
